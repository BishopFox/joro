...
Limited automated coverage (`go test ./...` — `internal/callback/listeners_test.go`, `internal/proxy/statusfilter_test.go`, and `internal/detect/rules_meta_test.go`, which pins the rule-copy conventions: every built-in has a distinct non-empty `Description`, names are sentence case with no trailing period, IDs and names are unique, and the analyzer registry resolves in both directions); otherwise manual verification:
