...
Limited automated coverage — `go test ./...` currently runs only `internal/callback/listeners_test.go`, `internal/proxy/statusfilter_test.go`, `internal/proxy/noise_test.go` (pins that the shipped OHTTP glob catches Mozilla's relays without swallowing other hosts on the same Fastly domain — a noise match is silent, so an over-broad default would hide a target with no trace), `internal/api/handlers_configs_test.go` (pins that the v6 backfill is additive, idempotent, leaves a nil `DetectConfig` nil, skips current-version files, and stays consistent with the shipped defaults — it rewrites saved engagement data, so a resurrected entry would be silent), and `internal/detect/parse_ohttp_test.go` (pins that OHTTP bodies skip as `content-type` rather than `binary`, that `NoteSkipped` counts only genuine blind spots, and — the negative control — that the same bytes under `text/plain` *are* still caught by the binary sniff). The `internal/detect/rules_meta_test.go` described in earlier revisions of this file **does not exist**; the rule-copy conventions in `rules_builtin.go` are currently unenforced, and writing that test is an open task. Otherwise, manual verification:
...
8. Manipulate (WS): connect to `wss://echo.websocket.events/`, send text/binary/ping, verify echo, disconnect. The binary (hex) send path is the regression guard for the shared `lib/bytes.ts` helpers.
8b. OHTTP / binary bodies:
    - Proxy Firefox and browse for a minute → **no** `mozilla-ohttp*.fastly-edge.com` rows in History or the Sitemap, and the Detect tab's amber "N unreadable" badge does not climb from them.
    - Load a `.joro` written before v6 → the OHTTP pattern appears in Settings → Project → Filtering → Noise Filter, with the rest of the saved list untouched. Delete an unrelated default, save, reload → it stays deleted (proves the backfill is targeted, not a union). Re-save → the file is `"version": 6`.
    - Delete the OHTTP noise pattern, browse again → the rows appear, and the badge *still* does not climb (proves the `SkipContentTypes` path independently of the noise filter). Select a row → the **Render** tab shows the envelope table (key ID, named KEM/KDF/AEAD, enc + ciphertext lengths) above a hex dump; the **Raw** tab is unchanged. Fetch a gateway's `application/ohttp-keys` URL through the proxy → it decodes to a key list with a full public key.
    - Negative controls: HTML, JSON (pretty-print toggle still works), and an image render exactly as before; an `application/octet-stream` body shows a hex dump with **no** OHTTP table.
