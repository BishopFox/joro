...
func analyzerRules() []Rule {
	...
	return []Rule{
		...
		an("cache-control-authenticated", "Authenticated response is cacheable", "cacheControlAuthenticated",
			CategoryHeaders, SeverityInfo,
			"The request carried credentials and the response lacks no-store, so a shared cache may serve one user's private data to another. Reported at low severity because a CDN in front of the app often makes this benign."),
		...
		an("cookie-missing-samesite", "Cookie missing SameSite", "cookieMissingSameSite",
			CategoryCookies, SeverityInfo,
			"No SameSite attribute. Low severity because browsers now default to Lax."),
